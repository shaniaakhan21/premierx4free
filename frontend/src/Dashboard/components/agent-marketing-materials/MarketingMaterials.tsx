...
const MarketingMaterials: React.FC = () => {
    ...
    const categories = useMemo<Pick<MarketingMaterialsCategory, '_id' | 'name'>[]>(() => Object.entries(data?.data?.reduce<{ [key: string]: string }>((a, c) => ({ ...a, [(c.category as MarketingMaterialsCategory)._id]: (c.category as MarketingMaterialsCategory).name }), {}) ?? {}).map(([_id, name]) => ({ _id, name })), [data])
