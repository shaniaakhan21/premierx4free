...
import { useMarketingMaterialsCategories, useMarketingMaterialsSummary } from "../../../../../services/admin";
...
function Categories(props: CategoryData): JSX.Element {
  ...
  const summaryIndexed = useMemo(() => {
    ...
    const cats: {
      [key: string]: Pick<MarketingMaterialsCategory, '_id' | 'name'> & {
        count: number,
        documents: MarketingMaterial[]
      }
    } = {};
        ...
        {summaryIndexed[category._id]?.documents?.length <= maxElements ?
          <Row className={classes.contentContainer_row}>
            {summaryIndexed[category._id]?.documents?.map((document, index: any) =>
              <Col key={document._id} xs={5} lg={containerWidth} className={classes.containerContent_contentUnit}
                   style={{ padding: 0 }}>
                <MarketingDocumentPreview fileName={document?.document} alt={document?.head}
                                          className={classes.containerContent_PdfPptIcon} />
            ...
            <Col xs={5} lg={containerWidth} className={classes.containerContent_contentUnitUploadMore}
                 onClick={() => setShowUploadMore(category)} style={{ position: "relative", right: "0" }}>
          ...
          <>
            <Row className={classes.contentContainer_row}>
              {summaryIndexed[category._id]?.documents.slice(0, maxElements)?.map((document, index: any) => <Col xs={5}
                                                                                                                 key={document._id}
                                                                                                                 lg={containerWidth}
                                                                                                                 className={classes.containerContent_contentUnit}
                                                                                                                 style={{ padding: 0 }}>
                <MarketingDocumentPreview fileName={document?.document} alt={document?.head}
                                          className={classes.containerContent_PdfPptIcon} />
                <div className={classes.contentUnit_buttons}>
            ...
            {viewMore ? <Row className={classes.contentContainer_row}>
              {summaryIndexed[category._id]?.documents.slice(maxElements, summaryIndexed[category._id]?.documents.length - 1)?.map((document, index: any) =>
                <Col xs={5} key={document._id} lg={containerWidth} className={classes.containerContent_contentUnit}
                     style={{ padding: 0 }}>
                  <MarketingDocumentPreview fileName={document?.document} alt={document?.head}
                                            className={classes.containerContent_PdfPptIcon} />
                  <div className={classes.contentUnit_buttons}>
