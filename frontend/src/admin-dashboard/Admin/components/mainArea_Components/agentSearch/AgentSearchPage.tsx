...
import { AgentStatus } from "../../../../../models/agentProfile.model";
    ...
    <div className={classes.calculationPage_mainContainer}>
      ...
      <TableContainer component={Paper} style={{ marginTop: 20, marginBottom: 20 }}>
        <Table sx={{ minWidth: 650 }} aria-label="simple table">
          ...
          <TableBody>
            {isLoading
              ? [...Array(8).keys()].map((k) => <TableRow key={k}>
                  ...
                  <StyledTableCell align="center" onClick={() => navigate(`/admin/agents/${row.user.userId}`)}>
                    {row.nda ?
                      <Chip style={{ margin: 2 }} icon={<DoneIcon sx={{ fontSize: 24, color: 'white' }} />} label="NDA"
                            color="success" /> : null}
                    {row.contract ?
                      <Chip style={{ margin: 2 }} icon={<DoneIcon sx={{ fontSize: 24, color: 'white' }} />}
                            label="Contract" color="success" /> : null}
                  </StyledTableCell>
                </TableRow>
              ))}
          </TableBody>
