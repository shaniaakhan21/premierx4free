...
function CalculationPage(): JSX.Element {
  ...
  return (
    <div className={classes.calculationPage_mainContainer}>
      {(showCreateContract || showEditContract) &&
        <CreateContract contract={showEditContract} onClose={(shouldReload) => {
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end' }}>
        <TextField
          style={{ minWidth: 400 }}
          id="query" label="Search" InputLabelProps={{ shrink: true }} variant="outlined" autoComplete='search'
        ...
        <div style={{ flex: 1 }} />
        <Button style={{ backgroundColor: '#64b5f6' }} startIcon={<AddIcon />}
                onClick={() => setShowCreateContract(true)} variant='contained' color="primary" aria-label="add">
          &nbsp;Add new
        </Button>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Pagination count={data?.data?.count ? Math.ceil(data?.data?.count / limit) : 0} page={(skip / limit) + 1}
